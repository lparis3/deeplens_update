...
def dlu_sim_1(Instrument,z_limits = None):
...
class SkipSimulation1(Exception):
        pass
...
def dlu_sim_2(DM_Type,Instrument,et_quotient,bands,source_images,source_mag,deflector_images,deflector_mag, redshifts, raw_src, raw_dfr,start_time,band_labels,Interlopers = True, lens_light = False):  
    ...
    def CDM_constructor(zsource, zlens, M_host, log_mlow,log_mhigh,Host_gamma, LOS_Norm):
        ...
        if (2*theta_E) / bands[0]['pixel_scale'] > 128:
            print("Ignoring simulation: Theta_E > 64 pixels.")
            raise SkipSimulation2
    ...
    while good == False and attempt <= max_attempts:
        try:
            ...
            print('Success!')
            good = True
        except SkipSimulation2 as e:
            raise SkipSimulation1            
    ...
    try:
        ...
        np.save(f'Range_error_data_{np.random.uniform(0,10)}.npy',np.array([Macro_kwargs_list[0]['theta_E'],redshifts,lens_mass,host_mass]))
    ...
    SNR_brightest = [np.sqrt(np.max(img_g_noise)),np.sqrt(np.max(img_r_noise)),np.sqrt(np.max(img_i_noise))] #SNR of brightest pixel in each band
    ...
    DL = (cosmology.comoving_distance(zdeflector).value)/1000
    hf.create_dataset(f'images/strong_lens_{i}/d_l', data=np.array([str(DL),'Comoving distance to deflector galaxy in Gpc'],dtype=dt),dtype=dt)
    DS = (cosmology.comoving_distance(zsource).value)/1000
    hf.create_dataset(f'images/strong_lens_{i}/d_s', data=np.array([str(DS),'Comoving distance to source galaxy in Gpc'],dtype=dt),dtype=dt)
    DLS = DL-DS
    hf.create_dataset(f'images/strong_lens_{i}/d_ls', data=np.array([str(DLS),'Comoving distance between deflector and source galaxy in Gpc'],dtype=dt),dtype=dt)
